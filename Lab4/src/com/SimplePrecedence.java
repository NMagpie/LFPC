...
public class SimplePrecedence {
    ...
    //TODO ask about cycles in the grammar
    ...
    //TODO some grammars have two "e", what does it mean?
    ...
    private static void parseInput(StringBuilder stringToCheck) {
        ...
        while (stringToCheck.compareTo(S) != 0) {
            ...
            String toReplace = transformElement(stringToCheck.substring(start, end + 1), startCh, endCh);
    ...
    private static String transformElement(String element, char start, char end) {

        //System.out.println(element+" "+start+" "+end);

        element = element.substring(1, element.length() - 1);

        element = element.replace("=", "");

        if (reverseGrammar.containsKey(element)) element = element.replace(element, reverseGrammar.get(element).toString());
        else return null;
